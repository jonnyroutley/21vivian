...
import { createReview, type NewReview, type Review } from "@/actions/reviews";
...
function ReviewRow({
	...
	created_at,
	idx,
}: Omit<components["schemas"]["Review"], "id" | "is_archived"> & {
	idx: number;
}) {
	return (
		<div
				...
				<form
					...
					action={async (formData: FormData) => {
						...
						const parsed = inputReviewSchema.safeParse({
							name,
							stars: parseInt(stars as string, 10),
							description,
							title,
						});
								...
								<div className="flex flex-row-reverse justify-end">
									{Array.from(Array(5)).map((_, idx) => (
										<>
											<input
												key={`star-${
													// biome-ignore lint/suspicious/noArrayIndexKey: they will be unique
													idx
												}`}
												type="radio"
												name="stars"
