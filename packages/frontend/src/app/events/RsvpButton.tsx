...
export function RsvpButton({ eventId }: { eventId: number }) {
	...
	const [canJoinEvent, setCanJoinEvent] = useState(
		!EventStorage.localSavedEvents.includes(eventId),
	);
			...
			>
				<form
					action={async (formData: FormData) => {
						...
						const eventIdHidden = parseInt(
							formData.get("eventIdHidden") as unknown as string,
							10,
						);
				...
				>
					<DialogHeader>
						...
						<DialogDescription className="text-xl">
							We can&apos;t wait to see you. Please let us know your name first.
							NO lollygagging.
						</DialogDescription>
					</DialogHeader>
					<div className="grid gap-4 py-4">
						<div className="grid grid-cols-4 items-center gap-4">
							...
							<Input
								id={`name`}
								name="name"
								className="col-span-3 text-xl text-neutral-900"
							/>
							<input
								id={`eventIdHidden`}
								name="eventIdHidden"
