...
import { ConsumptionRecord } from "@/actions/energy"
import {
  ChartConfig,
  ChartContainer,
  ChartLegend,
    ...
    <ChartContainer config={chartConfig} className="min-h-[300px] w-full pr-10 pt-20">
      <ResponsiveContainer>
        <BarChart accessibilityLayer data={transformedData} barGap={4}>
          ...
          <XAxis dataKey="datetime" tickLine={true} tickMargin={20} axisLine={false} angle={-20} />
          <YAxis />
          <ChartTooltip labelClassName="text-black" content={<ChartTooltipContent />} />
          ...
          <Bar
            ...
            stackId="a"
          />
          <Bar dataKey="gas" fill="var(--color-gas)" radius={[4, 4, 0, 0]} stackId="a" />
        </BarChart>
      </ResponsiveContainer>
    </ChartContainer>
  )
